#!/usr/bin/env python3
"""
Experiment 08: Prompt Injection Detection

Goal: Beat the PINT benchmark (Lakera Guard: 95.22%)

Method: Attribution graph analysis
- Computes causal influence between features
- Injection creates diffuse, scattered graphs
- Benign prompts have focused causal pathways

Discriminative metrics (Cohen's d > 1.0):
- top_100_concentration: Injection LOWER
- n_active: Injection HIGHER
- n_edges: Injection HIGHER
- mean_influence: Injection LOWER

Two-phase workflow:
1. Modal GPU computes attribution metrics → pint_metrics.json
2. This script loads metrics, calibrates, evaluates, saves results

Usage:
    # First: Run attribution on Modal
    modal run modal_pint_benchmark.py

    # Then: Analyze locally
    python experiments/08_injection_detection.py
"""
...
from hallucination_detector.injection_detector import (
...
def run_injection_detection_experiment(metrics_path: Path = None):
    ...
    # Initialize storage
    experiment_path = Path(__file__).parent / "08_injection_detection"
    ...
    if metrics_path is None:
        ...
        possible_paths = [
            Path(__file__).parent.parent / "pint_metrics.json",
            Path(__file__).parent.parent / "_meta" / "data" / "pint_metrics.json",
            Path(__file__).parent / "data" / "pint_metrics.json",
        ]
        ...
        if metrics_path is None:
            ...
            print("  modal run modal_pint_benchmark.py")
            print()
